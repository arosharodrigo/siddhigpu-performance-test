...
APP_CPU_MT="${JVM} ${JOPT} -jar ${JAR} --enable-async true --tps 100000 --enable-gpu false --usecase ${u} --execplan ${p} --execplan-count ${x} --usecase-count ${c} --ringbuffer-size ${r} --threadpool-size ${t} --events-per-tblock ${b} --batch-max-size ${Z} --batch-min-size ${z} --strict-batch-scheduling ${s} --work-size ${w} --selector-workers ${l} --use-multidevice false --device-count 0 --input-file ${DEBS_FILE}"

#APP_GPU_SD="${JVM} ${JOPT} -jar ${JAR} --tps 10000 --enable-async true --enable-gpu true --usecase ${u} --execplan ${p} --execplan-count ${x} --usecase-count ${c} --ringbuffer-size ${r} --threadpool-size ${t} --events-per-tblock ${b} --batch-max-size ${Z} --batch-min-size ${z} --strict-batch-scheduling ${s} --work-size ${w} --selector-workers ${l} --use-multidevice false --device-count 0 --input-file ${DEBS_FILE}"
