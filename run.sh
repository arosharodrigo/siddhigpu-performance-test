...
#JDB=/home/arosha/install/java/jdk1.6.0_29/jre/bin/java -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=5005
JVM=/home/arosha/install/java/jdk1.8.0_45/jre/bin/java
...
JOPT="-Xms2g -Xmx6g -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=5005"
LOG=logs/${p}
...
#APP_CPU_MT="${JVM} ${JOPT} -jar ${JAR} --enable-async true --enable-gpu false --usecase ${u} --execplan ${p} --execplan-count ${x} --usecase-count ${c} --ringbuffer-size ${r} --threadpool-size ${t} --events-per-tblock ${b} --batch-max-size ${Z} --batch-min-size ${z} --strict-batch-scheduling ${s} --work-size ${w} --use-multidevice 0 --device-count 0 --input-file ${DEBS_FILE}"
